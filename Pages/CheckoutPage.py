...
class CheckoutPage():
    ...
    def complete_checkout_options(self):
        self.driver.find_element(By.XPATH, self.guest_checkout_radiobutton_xpath).click()
        self.driver.find_element(By.XPATH, self.continue1_xpath).click()
    ...
    def enter_billing_details(self, fname, lname, email, tele, add1, city, pcode, country, state):
        ...
        country_val = self.driver.find_element(By.XPATH, self.country_xpath)
        time.sleep(3)
        dropdown1 = Select(country_val)
        time.sleep(3)
        dropdown1.select_by_visible_text(country)

        state_val = self.driver.find_element(By.XPATH, self.region_xpath)
        time.sleep(3)
    ...
    def complete_billing_details(self, fname, lname, email, tele, add1, city, pcode, country, state):
        ...
        self.driver.find_element(By.XPATH, self.continue2_xpath).click()

    def complete_delivery_method(self):
        self.driver.find_element(By.XPATH, self.continue4_xpath).click()

    def complete_payment_method(self):
        self.driver.find_element(By.XPATH, self.tnc_xpath).click()
        self.driver.find_element(By.XPATH, self.continue5_xpath).click()

    def confirm_order_method(self):
        #self.total_amount = self.driver.find_element(By.XPATH, self.total_xpath)
